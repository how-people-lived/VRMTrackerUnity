...
namespace VRMTracker.Visual
{
    ...
    {
        // 既定はオフ。素の MToon（アニメ調セルシェーディング）をそのまま見せる。
        // ポストプロセスは色味/明暗を変えて雰囲気が崩れやすいため、必要な人だけ Inspector で有効化。
        [Header("ポストプロセス（既定オフ＝自然なアニメ調）")]
        [SerializeField] bool  enablePostProcessing = false;
        [SerializeField] bool  tonemapACES          = false;
        [SerializeField] bool  enableBloom          = false;
        [SerializeField] bool  enableColorGrading   = false;
        [SerializeField] bool  enableVignette       = false;
        ...
        [SerializeField] bool  enableRimLight = false;
        ...
        [Header("陰影（MToon）")]
        [SerializeField] bool  tuneShade = false;
        ...
        [Header("バックライト（リム分離光）")]
        [SerializeField] bool  enableBackLight = false;
        ...
        // ── 設定パネルからの実行時トグル ─────────────────────────────────────────
        public bool PostProcessingOn { get; private set; }
        public void SetPostProcessing(bool on)
        {
            PostProcessingOn = on;
            if (_cam == null) return;
            if (on && _volume == null) BuildVolume();
            if (_volume != null) _volume.gameObject.SetActive(on);
            _cam.GetUniversalAdditionalCameraData().renderPostProcessing = on;
        }
        ...
        public void Setup(Camera cam)
        {
            ...
            if (cam == null) return;
            _cam = cam;
            PostProcessingOn = enablePostProcessing;

            var data = cam.GetUniversalAdditionalCameraData();
            data.renderPostProcessing = enablePostProcessing;   // オフなら確実に無効化
